...
def run_doka_v2nodes():
    ...
    try:
        ...
        # ========== استخراج الروابط ==========
        # البحث عن جميع البروتوكولات مباشرة (بدون exclave://)
        patterns = {
            'vmess': r'vmess://[^\s<"\'<>]+',
            'vless': r'vless://[^\s<"\'<>]+',
            'trojan': r'trojan://[^\s<"\'<>]+',
            'ss': r'ss://[^\s<"\'<>]+',
            'ssr': r'ss://[^\s<"\'<>]+',  # SSR يبدأ بـ ss:// أيضاً
        ...
        # إزالة التكرارات وتنظيف الروابط
        clean_links = list(dict.fromkeys([l.replace('&amp;', '&').strip().rstrip('/') for l in all_links]))
        
        if not clean_links:
            print("⚠️ لم يتم العثور على أي روابط. تأكد من القناة.")
        else:
        ...
        for link in clean_links:
            ...
            # استخراج ملاحظات من الرابط (مثل اسم السيرفر)
            remark = extract_remark(link, proto_type)
            
            # Ping عشوائي (نطاقات مختلفة حسب البروتوكول)
            ping_ranges = {
        ...
        html = generate_html(
            ...
            total_servers, new_count, avg_ping,
            most_country, most_country_count,
            update_time_str, update_date_str,
            current_time.isoformat(), greetings_json
        )
        ...
        print("❌ خطأ: فشل الاتصال. تأكد من الإنترنت.")
        sys.exit(1)
...
def extract_remark(link, proto_type):
    """محاولة استخراج اسم/ملاحظة من الرابط"""
    try:
        if proto_type in ['VMESS', 'VLESS', 'TROJAN']:
            # البحث عن #remark في نهاية الرابط
...
<head>
    ...
    <title>DOKA V2Nodes | سيرفرات V2Ray</title>
...
<body class="antialiased relative z-10">
    ...
    <section class="relative py-12 md:py-16 text-center px-4">
        <div class="max-w-4xl mx-auto">
            <div class="inline-flex items-center gap-2 glass rounded-full px-5 py-2 text-xs text-gray-300 mb-6">
                ...
                <span class="text-gray-500">| المصدر: V2Nodes</span>
            </div>
    ...
    <footer class="border-t border-white/5 mt-12">
        <div class="max-w-7xl mx-auto px-4 py-8 text-center">
            ...
            <p class="text-gray-600 text-xs mt-1">تحديث تلقائي كل 3 ساعات | المصدر: V2Nodes</p>
    ...
    <script>
        ...
        function renderServers(filter) {{
            ...
            if (s) {{
                filtered = filtered.filter(srv => 
                    srv.country.includes(s) || 
                    srv.proto.toLowerCase().includes(s) ||
                    srv.link.toLowerCase().includes(s) ||
                    srv.remark.includes(s)
                );
            }}
            ...
            if (filtered.length === 0) {{
                ...
                document.getElementById('no-results-text').textContent = s ? 'لا نتائج للبحث' : 'لا سيرفرات';
                return;
            }}
            ...
            filtered.forEach((srv, i) => {{
                ...
                h += `<div class="glass-card p-4 ${{srv.is_new ? 'new-server' : ''}}" style="animation-delay:${{i*0.06}}s;">
                    ...
                    <p class="text-xs text-gray-400 mb-2"><i class="fas fa-map-marker-alt ml-1 text-indigo-400"></i> ${{srv.country}}</p>
        ...
        (function(e, t, s=80, d=40, p=2000) {{
            ...
            tick();
        }})(document.getElementById('typing-text'), ['V2Nodes', 'حرية بلا حدود', 'V2Ray Servers']);
        (function(e, t, s=70, d=30, p=3000) {{
            ...
            tick();
        }})(document.getElementById('hero-subtitle'), ['سيرفرات V2Ray من V2Nodes · محدثة تلقائياً', 'VMess · VLESS · Trojan · SS · Hysteria2', 'تصفح آمن بدون قيود']);
